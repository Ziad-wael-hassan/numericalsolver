...
SolverResult BisectionSolver::solve(const SolverConfig &config) const {
    ...
    SolverResult result;
    result.methodName = toDisplayName(MethodId::Bisection);
    ...
    for (int iteration = 1; iteration <= config.maxIterations; ++iteration) {
        ...
        if (std::abs(fxr) <= config.tolerance ||
            ...
            result.message = "Root bracket narrowed successfully.";
            if (!config.showAllIterations) {
                result.rows = {lastRow};
            }
            return result;
